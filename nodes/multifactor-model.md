...
systematic risk rather than to one, with a residual left over that is specific to the asset
and uncorrelated with every factor.
...
through a shared exposure to oil prices. Allowing several factors at once recovers that
distinction and lets a portfolio's risk be decomposed factor by factor instead of left as one
undifferentiated number. The single-index model needs this node next, since it is the special
case reached by collapsing every factor here into one.
