---
id: shortfall-probability
title: Shortfall probability
domains: [fin-eng, stats]
status: drafted
requires: []
spends: []
anchor: [ifoa.cm2.2.1-1]
vault_articles: []
vault_sources: []
taught_in: null
---

## Definition

The shortfall probability is the probability that an accumulated fund, or an investment
return, falls below a stated target level by a given date.

## The expression

$$
\mathrm{SP} = P(A_T \lt L)
$$

Here $A_T$ is the random accumulated value of the fund at time $T$, $L$ is the target level the
investor has set, and $\mathrm{SP}$ is the probability that the fund fails to reach it.

## Why this node exists

An expected return alone says nothing about how likely an investor is to miss the level they
actually need, whether that is a pension fund's liability or a savings target, and shortfall
probability supplies that answer directly. It is one of several ways of summarising downside
risk, alongside the size of the shortfall and the volatility of returns, and comparing
investment risk measures needs it next as one of the candidates that comparison sets against
the others.
