...
def render_report(corpus: Corpus, index_built: str) -> str:
    ...
    anchor_intro = (
        ...
        "records which syllabus documents the corpus already holds. It "
        "is keyed by anchor body, a broader grouping than the "
        "`<body>.<subject>` anchor documents the table below lists, so "
        "one row there can cover several rows here, as it does for the "
        "University of Pretoria entries. A reader checking whether a "
        "document below is already held should look up its issuing "
        "body in `sources/syllabi.yaml`, and should expect that file's "
        "row count to differ from the table's rather than match it."
    )
