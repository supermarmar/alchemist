...
from .model import REPO, Corpus, Objects, Spend, load_corpus, load_objects
...
def render_node_page(node, corpus: Corpus, objects: Objects) -> str:
    ...
    # Group by object and trigger on one object being spent in more than one
    # domain, which is what "the same thing under several names" means. The
    # earlier trigger was the node touching more than one domain, so a node
    # spending four hazard aliases and one survival alias rendered all five in
    # one undifferentiated list, two of them labelled `credit`, under a heading
    # promising a single object. Most Phase 3 nodes spend more than one object,
    # so nearly every page would have inherited that.
    by_object: dict[str, list[Spend]] = {}
    for spend in node.spends:
        by_object.setdefault(spend.object, []).append(spend)

    multi = {oid: spends for oid, spends in by_object.items() if len(spends) > 1}
    if multi:
        rows = []
        for oid, spends in multi.items():
            obj = objects.by_id[oid]
            for spend in spends:
                alias = obj.for_domain(spend.domain)
                # alias.symbol is deliberately not passed through _esc: it is
                # TeX destined for KaTeX, not HTML destined for a reader, and
                # escaping it would hand the renderer "&#x5C;mu_x" instead of
                # "\mu_x". It comes from notation/objects.yaml, reviewed repo
                # content rather than arbitrary author input, and check 1
                # already constrains what a node may spend. Wrapping it the
        ...
        parts += [
            "<h2>One object, several names</h2>",
            "<table><thead><tr><th>Object</th><th>Domain</th><th>Symbol</th>"
            "<th>Called</th></tr></thead><tbody>" + "".join(rows) + "</tbody></table>",
        ]
